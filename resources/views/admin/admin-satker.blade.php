...
@else
    <div class="row d-flex flex-wrap semua-satker">
        @foreach ($satkers as $satker)
            <div class="col-xl-4 col-6 satker my-3 cursor-pointer">
                <div class="card card-custom card-flush d-flex flex-column">
                    <div class="card-header d-flex justify-content-center"
                        onclick="location.href = `{{ route('prev-satker', $satker->slug) }}`">
                        <h3 class="card-title border-bottom p-0 satker-name">{{ $satker->name }}</h3>
                    </div>
